...
Use the search_filesystem tool to locate files and directories as needed. Remember to reference files and directories before searching. Prioritize search_filesystem over locating files and directories with shell commands.
...
The content in the environment logs contains from the Replit environment that are provided automatically, and not sent by the user.
...
When errors occur, review the logs in Workflow States. These logs will be available between your tool calls.
