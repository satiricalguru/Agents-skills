...
How to access the user's pc:

I run a private Flask dropbox on my PC, tunneled via ngrok. Use it to write code, configs, or any file straight to my machine. All paths are relative to my drop directory.

BASE_URL = your base url from the terminal
AUTH_KEY = same thing with the auth key

Always send the header: X-Auth: <AUTH_KEY>

Endpoints:
- POST   {BASE_URL}/write?path=<relpath>   body = raw file contents (overwrites)
- POST   {BASE_URL}/append?path=<relpath>  body = raw bytes to append
- GET    {BASE_URL}/read?path=<relpath>    returns raw bytes
- GET    {BASE_URL}/list?path=<reldir>     returns JSON {root, items:[{name,is_dir,size}]}
- DELETE {BASE_URL}/delete?path=<relpath>  removes file or directory tree

Rules:
- Use forward slashes in paths, e.g. path=project/src/main.py
- Parent directories are auto-created on /write and /append
- Path traversal (..) is blocked server-side
- Default file extension follows the language (.py, .cpp, .ps1, etc.) — pick correct one
- After writing anything non-trivial, call /list on its parent to confirm, and /read it back if you want to verify contents

curl examples:
  # write a file
  curl -X POST "{BASE_URL}/write?path=project/main.py" \
       -H "X-Auth: {AUTH_KEY}" \
       --data-binary @- <<'EOF'
  print("hello from ENI")
  EOF

  # list a directory
  curl -H "X-Auth: {AUTH_KEY}" "{BASE_URL}/list?path=project"

  # read a file
  curl -H "X-Auth: {AUTH_KEY}" "{BASE_URL}/read?path=project/main.py"

  # delete
  curl -X DELETE -H "X-Auth: {AUTH_KEY}" "{BASE_URL}/delete?path=project/main.py"

Python helper (preferred for multi-file drops):
  import requests
  BASE = "<BASE_URL>"
  AUTH = {"X-Auth": "<AUTH_KEY>"}
  ...
  def read(path):
      r = requests.get(f"{BASE}/read", params={"path": path}, headers=AUTH)
      r.raise_for_status(); return r.content
  def ls(path=""):
      r = requests.get(f"{BASE}/list", params={"path": path}, headers=AUTH)
      r.raise_for_status(); return r.json()
  def rm(path):
      r = requests.delete(f"{BASE}/delete", params={"path": path}, headers=AUTH)
      r.raise_for_status(); return r.json()

When I ask you to "drop this on my PC" or "write this to my box" or "write code" or anything with code and writing, use these endpoints. Confirm each write with the returned path and size, then move on.